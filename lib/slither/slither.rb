class Slither
  ...
  def self.parse(filename, definition_name, opts={}, &block)
    raise ArgumentError, "File #{filename} does not exist." unless File.exists?(filename)

    file_io = File.open(filename, 'r')
    parseIo(file_io, definition_name, opts, &block)
  end

  def self.parseIo(io, definition_name, opts={}, &block)
    definition = definition(definition_name)
    raise ArgumentError, "Definition name '#{definition_name}' was not found." unless definition
    parser = Parser.new(definition, io)
    definition.options[:by_bytes] ? parser.parse_by_bytes(opts, &block) : parser.parse(opts, &block)
  end
